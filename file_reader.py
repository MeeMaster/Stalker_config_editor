...
class Reader:
    ...
    def get_item_types(self):
        # pattern = re.compile("^\[.*\]")
        path = os.path.join(self.dirpath, "configs", "items", "trade", "presets", "trade_presets.ltx")
            ...
            for line in infile:
                ...
                if match is not None:
                    ...
                    if ":" in line:
                        ...
                        self.item_groups[name] = members.split(";")[0].strip().split(",")
                        continue
                    self.item_types[name] = []
                    continue
                self.item_types[name].append(line.strip())

    def read_file(self, path):
        with open(path, "r", encoding="ISO-8859-1") as infile:
            file_entry = FileEntry(remove_dirpath(path, self.dirpath))
            entry = None
            counter = 0
            lineno = 0
            for line in infile:
                ...
                if match is not None:
                    ...
                    if ":" in line:
                        ...
                        self.inheritances[name] = parents
                    ###

                    entry = Entry(name, parents, path, lineno)
                    entry_line_no = 0
                    self.definitions[name] = {"file": path, "parents": parents}
                    continue
                if entry is None:
                ...
                if match is not None:
                    ...
                    prop_name = prop
                    parent = name
                    if name not in self.object_lists:
                        self.object_lists[name] = []
                    self.object_lists[name].append(prop_name)
                if "," in values:
                    values = [a.strip() for a in values.split(",")]
                else:
                    values = [values]
                ###
                line_entry = LineEntry(remove_dirpath(path, self.dirpath), entry_line_no, entry.name, prop_name,
                                       parent, values, comments)
    ...
    def read_all_files(self):
        for x, y, z in os.walk(os.path.join(self.dirpath, "configs",  "items")):
            for f in z:
    ...
    def read_items(self):
        path = os.path.join(self.dirpath, "configs",  "defines.ltx")
        if not os.path.exists(path):
            print("Wrong folder!")
            return
        file_entry = self.read_file(path)
